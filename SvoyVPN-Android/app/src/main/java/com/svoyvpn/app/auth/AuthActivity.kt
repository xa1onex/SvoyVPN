...
import com.svoyvpn.app.WebViewActivity
...
class AuthActivity : FragmentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // If already logged in, go straight to WebView
    ...
    private fun setupWebView() {
        ...
        webView.webViewClient = object : WebViewClient() {
            override fun shouldOverrideUrlLoading(view: WebView?, url: String?): Boolean {
                if (url?.startsWith("tg://") == true || url?.startsWith("https://t.me/") == true) {
    ...
    private fun loadAuthPage() {
        val html = buildAuthHtml()
        webView.loadDataWithBaseURL("https://xdoublegroup.online", html, "text/html", "utf-8", null)
    }

    private fun buildAuthHtml(): String {
        val botUsername = BuildConfig.BOT_USERNAME
        return """
<!doctype html>
<html lang="ru">
<head>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    :root {
      ...
      --border: rgba(0,0,0,0.08); /* Though bordered inputs are not very miniapp-like, we keep it subtle */
    }
    ...
    .polling {
      ...
      margin-top: 20px;
      padding: 14px;
      background: rgba(58,168,252,.08);
      border: 1px solid rgba(58,168,252,.2);
      border-radius: 12px;
      text-align: center;
    }
    ...
    .dots::after {
      content: '';
      animation: dotdot 1.5s infinite;
    }
    @keyframes dotdot {
      0%   { content: ''; }
      25%  { content: '.'; }
...
<div class="page">
  ...
  <div class="card">
    ...
    <div id="panelTg" class="panel active">
      ...
      <div class="tg-hint">
        Вы будете перенаправлены в Telegram. После нажатия «Старт» в боте вход произойдёт автоматически.
      </div>
    </div>
    ...
    <div id="panelEmail" class="panel">
      ...
      <div id="resetForm" style="display:none;">
        ...
        <button onclick="switchEmailTab('login')" style="width:100%;padding:12px;background:none;border:none;color:var(--muted);font-size:13px;cursor:pointer;margin-top:8px;-webkit-tap-highlight-color:transparent;">← Вернуться ко входу</button>
      </div>
      ...
      <div id="registerForm" style="display:none;">
        ...
        <div id="regStep2" style="display:none;">
          ...
          <button onclick="showRegStep1()" style="width:100%;padding:12px;background:none;border:none;color:var(--muted);font-size:13px;cursor:pointer;margin-top:8px;-webkit-tap-highlight-color:transparent;">← Изменить email</button>
        </div>
      </div>
...
<script>
  const API = 'https://xdoublegroup.online/api';
  ...
  function haptic(style) {
    if (window.AndroidAuth && window.AndroidAuth.vibrate) {
      window.AndroidAuth.vibrate(style);
    }
  }
  ...
  function notifyNative(token, userId) {
    ...
    if (window.AndroidAuth) {
      AndroidAuth.onLoginSuccess(token, userId || '');
    }
  }
  ...
  async function startTgLogin() {
    ...
    try {
      ...
      // Show polling indicator and start polling
    ...
    pollingInterval = setInterval(async () => {
      try {
        ...
        if (data.status === 'ok' && data.token) {
          ...
          if (window.AndroidAuth) {
            AndroidAuth.onLoginSuccess(data.token, '');
          }
  ...
  async function doLogin() {
    ...
    try {
      ...
      if (window.AndroidAuth) AndroidAuth.onLoginSuccess(data.token, data.userId + '');
    } catch(e) {
  ...
  async function doRegister() {
    ...
    try {
      ...
      if (data.token && window.AndroidAuth) {
        AndroidAuth.onLoginSuccess(data.token, (data.userId || '') + '');
      }
    } catch(e) {
      showError('registerError', 'Ошибка подключения');
    }
    ...
    inner class AndroidAuthBridge {
        ...
        fun onLoginSuccess(token: String, userId: String) {
            ...
            if (userId.isNotEmpty()) {
                try { tokenStorage.saveUserId(userId.toLong()) } catch (_: Exception) {}
            }
