            ...
            if not user:
                ...
                if migrated_data:
                    ...
                    await message.answer(
                        f"👋 <b>Рады видеть вас в нашем новом боте!</b>\n\n"
                        f"Ваши данные успешно перенесены из старого бота.{sub_info}{utm_bonus_info}{keys_info}\n\n"
                        f"Что изменилось? \n\n"
                        f"1. Для удобного использования теперь все можно делать в MiniAPP - просто нажми Открыть приложение \n"
                        f"2. Теперь никаких ключей, одна конфигрурацияn - все доступные локации без ограничений\n"
                        f"3. Мы поняли, что нам не хватало - стабильности, мы улучшили наши сервера теперь точно все стабильно!\n"
                        f"4. Начислили компенсацию и об скидках не забыли:)\n\n"
                        f"@majorka_wy - если что то не так перенеслось; /start - избавиться от этого сообщения",
                        parse_mode="HTML",
