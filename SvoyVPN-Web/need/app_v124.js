...
(function () {
  ...
  async function measurePing(serverId) {
    try {
      ...
      const r = await fetch('./api/ping?id=' + serverId, { cache: 'no-store' });
      ...
      if (sub && sub.isActive) {
        ...
        if (S.user && S.user.trialAvailable) {
          ...
          document.getElementById('btnActivateTrial').addEventListener('click', async function () {
            ...
            const d = await api('./api/trial/activate', {
              method: 'POST',
  ...
  async function deleteNews(newsId) {
    const d = await api('./api/news/delete', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
    ...
    if (btnSave) {
      btnSave.onclick = async () => {
        ...
        try {
          ...
          const resp = await fetch('./api/news/add', {
            method: 'POST',
            body: formData
          });
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadReferral() {
      if (!tg || !tg.initData) return;
      const d = await api('./api/referral?initData=' + encodeURIComponent(tg.initData));
      if (d && d.referralCode) {
        refLink = d.refLink;
...
// === WEBSITE AUTH INJECTION ===
const WEB_API = window.AppConfig?.apiBaseURL || 'https://xdoublegroup.online/api';
let webToken = localStorage.getItem('svoyvpn_web_jwt');

document.addEventListener('DOMContentLoaded', () => {
    // Hide all screens initially if we need auth
    // The original miniapp defaults screenVpn to active. We should hide it.
    
    if (!window.AndroidBridge && !window.webkit?.messageHandlers?.iOSBridge && !window.Telegram?.WebApp?.initData) {
        document.querySelectorAll('.screen').forEach(s => s.classList.remove('active'));
        let tabBar = document.getElementById('tabBar');
        if (tabBar) tabBar.style.display = 'none';

        window.WEB_LOGIN = async function() {
            const eml = document.getElementById('webAuthEmail')?.value;
            const pass = document.getElementById('webAuthPass')?.value;
            const err = document.getElementById('webAuthError');
            const btn = document.getElementById('webBtnLogin');
            ...
            try {
                let res = await fetch('https://xdoublegroup.online/api/auth/login', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                ...
                if (res.ok && data.token) {
                    webToken = data.token;
                    localStorage.setItem('svoyvpn_web_jwt', webToken);
                    loadWebApp();
                } else {
                    err.innerText = data.detail || 'Ошибка авторизации';
        ...
        let btnLgn = document.getElementById('webBtnLogin');
        if (btnLgn) btnLgn.addEventListener('click', window.WEB_LOGIN);

        if (webToken) {
            loadWebApp();
        } else {
            let sa = document.getElementById('screenAuth');
...
function loadWebApp() {
    ...
    // Inject webToken to android jwt variable for normal miniapp flow to bypass telegram data
    window.__androidJwt = webToken;

    // Reload entire state assuming the token is set and the original IIFE will use it.
    // wait, if we just set it, the old IIFE already ran, so we must just reload the page with jwt qs!
    if (!window.location.search.includes('jwt')) {
        window.location.href = window.location.pathname + '?jwt=' + webToken;
    } else {
       let sv = document.getElementById('screenVpn');
...
window.__androidLogout = function() {
    ...
    webToken = null;
    window.location.href = window.location.pathname; // Clear JWT and reload
};
