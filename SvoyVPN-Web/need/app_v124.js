...
(function () {
  ...
  /* ═══════ Theme ═══════
     SDK по умолчанию держит colorScheme = 'light' до реального Mini App.
     Вне Telegram initData пустой — иначе системная тёмная тема никогда не применится. */
  ...
  async function loadEsimPackages(country) {
    ...
    let url = '/api/esim/packages?country=' + encodeURIComponent(country);
    if (tg && tg.initData) url += '&initData=' + encodeURIComponent(tg.initData);
    const skipJwt = !!(tg && tg.initData);
    try {
      const d = await api(url, { method: 'GET', skipWebJwt: skipJwt });
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    /* Системная тёмная/светлая тема — только вне Mini App (в TG слушаем themeChanged) */
    if (!URL_THEME) {
      try {
    ...
    async function submitEsimBetaNotifyRequest(btnEl) {
      ...
      if (IS_ANDROID) {
        ...
        opts.skipWebJwt = true;
      } else if (localStorage.getItem('svoyvpn_web_jwt')) {
        /* fetch wrapper подставит Authorization */
      } else {
        showToast('Войдите или откройте приложение в Telegram');
        return;
      }
      opts.body = JSON.stringify(body);
      if (btnEl) btnEl.disabled = true;
      try {
        const d = await api('/api/esim/beta-notify', opts);
        if (d && d.status === 'ok') {
