...
(function () {
  ...
  function applyTheme() {
    ...
    // Sync header and background with Telegram theme
    if (tg) {
      // setHeaderColor — makes native Telegram header match the app bg
      if (tg.setHeaderColor) {
        try { tg.setHeaderColor('bg_color'); } catch (_) { }
      }
      // setBackgroundColor — keeps the area behind the webview matched too
      if (tg.setBackgroundColor) {
        try { tg.setBackgroundColor('bg_color'); } catch (_) { }
      }
      // For newer Bot API: set bottom bar color too
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (tg) {
      tg.ready();
      tg.expand();
      // Re-apply theme (including header/bg colors) on every theme change
